...
- [0) 启动（Kickoff）](#0-启动kickoff)
- [1) 系统架构与接口定义（系统设计）](#1-系统架构与接口定义系统设计)
- [2) 迭代计划与任务拆分（Backlog）](#2-迭代计划与任务拆分backlog)
- [3) 可行性--poc-迭代prototype-loop](#3-可行性--poc-迭代prototype-loop)
- [4) 原理图设计与评审（HW）](#4-原理图设计与评审hw)
- [5) PCB 布局与出板（HW）](#5-pcb-布局与出板hw)
- [6) BSP--Boot-与基础固件（SW-并行）](#6-bsp--boot-与基础固件sw-并行)
- [7) 首次样板 Bring-up（Integration）](#7-首次样板-bring-upintegration)
- [8) 驱动与中间件实现（迭代开发）](#8-驱动与中间件实现迭代开发)
- [9) 功能集成测试（Integration--hil）](#9-功能集成测试integration--hil)
- [10) 回归--release-candidaterc](#10-回归--release-candidaterc)
- [11) 试产（Pilot）与生产测试准备](#11-试产pilot-与生产测试准备)
- [12) 量产--维护production](#12-量产--维护production)
- [并行规则与沟通（保证流程顺畅）](#并行规则与沟通保证流程顺畅)
